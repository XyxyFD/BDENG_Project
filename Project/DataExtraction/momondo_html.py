...
html_folder = "Project/Html/momondoHTML"
...
for filename in os.listdir(html_folder):
    if filename.endswith(".html") and filename.startswith("momondo_"):
        file_path = os.path.join(html_folder, filename)
        
        # Dateinamen parsen, um Abflug-Flughafen und Datum zu extrahieren
        match = re.search(r"momondo_([A-Z]{3})_(\d{2})(\d{2})\.html", filename)
...
df.to_csv("Project/csv/momondo_data.csv", index=False, encoding="utf-8")

print("Fertig – Daten aus allen HTML-Dateien im momondoHTML-Ordner gesammelt und in momondo_data.csv gespeichert.")
